...
def get_distance_matrix(atoms):
    ...
    if pbc.any():
        ...
        cell = None
    dist_matrix = compute_distance_matrix(atoms.positions, cell)
    return dist_matrix
...
def freq_dist_change_mat(X, positions):
    X = X.copy()
    dists0 = compute_distance_matrix(positions)
    return np.array([compute_distance_matrix(x) for x in X+positions]) - dists0
